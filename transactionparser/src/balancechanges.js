...
var dropsToXDV = require('./utils').dropsToXDV
...
function parseXDVQuantity(node, valueParser) {
  ...
  return {
    ...
    balance: {
      counterparty: '',
      currency: 'XDV',
      value: dropsToXDV(value).toString()
    }
  }
}
...
function parseQuantities(metadata, valueParser) {
  var values = normalizeNodes(metadata).map(function(node) {
    if (node.entryType === 'AccountRoot') {
      return [parseXDVQuantity(node, valueParser)]
    } else if (node.entryType === 'DivvyState') {
